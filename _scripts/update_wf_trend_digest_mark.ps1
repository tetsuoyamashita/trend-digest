...
$connectionsJson = @'
{
  ...
  "Notion PATCH": { "main": [ [ { "node": "Respond 200", "type": "main", "index": 0 } ] ] }
}
'@
...
$wf = @{
  ...
  nodes = @(
    @{
      parameters = @{ httpMethod = 'POST'; path = 'trend-digest-mark'; responseMode = 'responseNode'; options = @{} }
...
$r = Invoke-RestMethod -Method Put -Uri "$base/workflows/$wfId" -Headers @{ 'X-N8N-API-KEY' = $apiKey; 'Content-Type' = 'application/json' } -Body $bodyBytes
"updated: id=$($r.id)  active=$($r.active)"
